...
def main():
    ...
    if all(col in column_names for col in ['instruction', 'output']):
        input_column_name = 'input' if 'input' in column_names else None
        instruction_column_name = 'instruction'
        output_column_name = 'output'
        train_on_inputs = False  # 标准 Alpaca 不训练 inputs
    else:
        raise ValueError('输入数据必须包含至少 instruction 和 output 字段')
    ...
    def generate_and_tokenize_prompt(data_point):
        instruction = data_point[instruction_column_name]
        input_text = data_point[input_column_name] if input_column_name else ""
        output_text = data_point[output_column_name]
        ...
        tokenized = tokenizer(full_prompt, truncation=True, max_length=data_args.block_size, padding=False)
        input_ids = tokenizer(prompt, truncation=True, max_length=data_args.block_size, padding=False)["input_ids"]
        user_prompt_len = len(input_ids)

        tokenized["labels"] = [-100] * user_prompt_len + tokenized["input_ids"][user_prompt_len:]
        return tokenized
    ...
    with training_args.main_process_first(desc="dataset map tokenization"):
        if not data_args.streaming:
            tokenized_datasets = raw_datasets.map(
                ...
                desc="Running tokenizer on Alpaca-style dataset",
            )
